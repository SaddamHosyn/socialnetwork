...
func InviteToGroupHandler(w http.ResponseWriter, r *http.Request) {
	...
	invitationID, err := db.CreateGroupInvitation(groupID, userID, inviteeID)
	if err != nil {
	...
	inviterName, err := db.GetUserNameByID(userID)
	if err != nil {
		log.Printf("Error getting inviter name: %v", err)
		inviterName = "Unknown User"
	}

	err = db.CreateGroupInvitationNotification(inviteeID, userID, inviterName, groupName, invitationID)
	if err != nil {
...
func RequestJoinGroupHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Get group details and creator
	var creatorID int
	var groupName string
	err = sqlite.GetDB().QueryRow(`SELECT creator_id, name FROM groups WHERE id = ?`, groupID).Scan(&creatorID, &groupName)
	if err != nil {
	...
	requesterName, err := db.GetUserNameByID(userID)
	if err != nil {
		log.Printf("Error getting requester name: %v", err)
		requesterName = "Unknown User"
	}

	err = db.CreateJoinRequestNotification(creatorID, userID, requesterName, groupName, requestID)
	if err != nil {
