...
func CreateGroupEventHandler(w http.ResponseWriter, r *http.Request) {
	...
	memberIDs, err := db.GetGroupMembersForNotification(groupID)
	if err != nil {
		...
		creatorName, err := db.GetUserNameByID(userID)
		if err != nil {
		...
		for _, memberID := range memberIDs {
			if memberID != userID {
				err = db.CreateGroupEventNotification(memberID, userID, creatorName, title, groupName, eventID)
				if err != nil {
					log.Printf("Error creating event notification for user %d: %v", memberID, err)
				}
			}
		}
	}
